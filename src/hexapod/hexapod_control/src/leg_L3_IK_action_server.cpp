...
public:
	...
	void executeCB(const hexapod_control::GaitGoalConstPtr& goal)
	{
        ...
        node.getParam("/hexapod/geometry/leg_L3/foot/x", default_foot_x);
        node.getParam("/hexapod/geometry/leg_L3/foot/y", default_foot_y);
        node.getParam("/hexapod/geometry/leg_L3/foot/z", default_foot_z);
        node.getParam("/hexapod/geometry/leg_L3/hip/x", default_hip_x);
        node.getParam("/hexapod/geometry/leg_L3/hip/y", default_hip_y);
        node.getParam("/hexapod/geometry/leg_L3/hip/z", default_hip_z);
...
private:
    ...
    geometry_msgs::Point calcHips(geometry_msgs::Twist twist)
    {
        ...
        std::vector<double> s = {Rb*cos(240*M_PI/180), Rb*sin(240*M_PI/180), 0}; // specific to L3
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "leg_L3_IK_action");
    ROS_INFO("Initialized ros...");

    SetIKAction actionServer("leg_L3_IK_action");
