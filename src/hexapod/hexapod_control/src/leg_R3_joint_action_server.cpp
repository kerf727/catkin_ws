...
public:
        ...
        actionName(name)
    {
        ...
        this->hipJointPublisher = node.advertise<std_msgs::Float64>("/hexapod/leg_R3/hip_joint_position_controller/command", 1);
        this->kneeJointPublisher = node.advertise<std_msgs::Float64>("/hexapod/leg_R3/knee_joint_position_controller/command", 1);
        this->ankleJointPublisher = node.advertise<std_msgs::Float64>("/hexapod/leg_R3/ankle_joint_position_controller/command", 1);

        ROS_INFO("Subscribing to FKPoseSolver service...");
        this->fkClient = node.serviceClient<hexapod_control::SolveFKPose>("/hexapod/leg_R3/fk");
    ...
    void jointStatesCB(const sensor_msgs::JointStateConstPtr& msg)
    {
        ...
        for (int i = 0; i < temp.name.size(); ++i)
        {
            ...
            if (name_i.find("R3") != std::string::npos)
            {
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "leg_R3_joint_action");

    SetJointAction actionServer("leg_R3_joint_action");
    ros::spin();
    return 0;
}