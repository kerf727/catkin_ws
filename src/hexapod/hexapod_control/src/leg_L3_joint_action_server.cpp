...
public:
        ...
        actionName(name)
    {
        ...
        this->hipJointPublisher = node.advertise<std_msgs::Float64>("/hexapod/leg_L3/hip_joint_position_controller/command", 1);
        this->kneeJointPublisher = node.advertise<std_msgs::Float64>("/hexapod/leg_L3/knee_joint_position_controller/command", 1);
        this->ankleJointPublisher = node.advertise<std_msgs::Float64>("/hexapod/leg_L3/ankle_joint_position_controller/command", 1);

        ROS_INFO("Subscribing to FKPoseSolver service...");
        this->fkClient = node.serviceClient<hexapod_control::SolveFKPose>("/hexapod/leg_L3/fk");
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "leg_L3_joint_action");

    SetJointAction actionServer("leg_L3_joint_action");
    ros::spin();
    return 0;
}